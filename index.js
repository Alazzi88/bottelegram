...
if (!token) {
  console.error("❌ خطأ: لم يتم العثور على توكن البوت! تأكد من إضافته في ملف .env.");
  process.exit(1);
}
if (!ADMIN_PHONE) {
  console.error("❌ خطأ: لم يتم العثور على رقم هاتف الأدمن! تأكد من إضافته في ملف .env.");
  process.exit(1);
}
if (!ADMIN_CHAT_ID) {
  console.warn("⚠️ تحذير: لم يتم تحديد ADMIN_CHAT_ID. سيحتاج الأدمن للتحقق عبر رقم الهاتف لمعرفة الـ Chat ID.");
}
...
let badKeywords = [
  "سكليف","سكلفت","اجازه","اجازة","مرضيه","طبي","طبيات","عذر","اعذار","سكل","تقرير","تقارير","غياب","غيابات","مرافق","صحي","مستثمر","يستثمر","استثمار","تداول","ارباح","ربح","فائده","فوائد","محفظه","توصيات","خساره","تعويض","بيتكوين","عملات","فوركس","تشفير","مضمون","مضمونه","دخلتمعشخص","نصب","احتيال","هكر","تهكير","اختراق","استرجاع","منحول","تحويل","بنكي","صراف","زواج","مسيار","خطابه","مطلقه","تعارف","خاص","كلمني","سحر","روحاني","الشيخ","علاج","ادله","اثبات","اثباتات","صوره","اسكرين","واتس","واتساب","تواصل","رقمي","سكس","اباحي","موجب","سالب","اسستثمر",
];

// --- الدوال المساعدة ---
function stripText(text) {
  if (!text) return "";
  let clean = text.replace(/[\u0640\u064B-\u065F\u0670]/g, "").replace(/[^\p{L}\p{N}]/gu, "");
  clean = clean.replace(/[أإآا]/g, "ا").replace(/[ةه]/g, "ه").replace(/[ىي]/g, "ي").replace(/[ؤ]/g, "و").replace(/[ئ]/g, "ي");
...
bot.on("message", async (msg) => {
    ...
    switch (adminState) {
        case 'awaiting_word_to_add':
            ...
            if (word && !badKeywords.includes(word)) {
                ...
                bot.sendMessage(chatId, `✅ تم إضافة الكلمة: *${word}*`, { parse_mode: "Markdown" });
            } else {
        ...
        case 'awaiting_word_to_remove':
            ...
            if (index > -1) {
                ...
                bot.sendMessage(chatId, `✅ تم حذف الكلمة: *${word}*`, { parse_mode: "Markdown" });
            } else {
                bot.sendMessage(chatId, `⚠️ الكلمة "*${word}*" غير موجودة في القائمة.`, { parse_mode: "Markdown" });
            }
            break;
    }
    adminState = null;
  ...
  const linkRegex = /(https?:\/\/)|(www\.)|(\.com|\.net|\.org|\.me)|(t\.me\/)|(@[\w]+)/i;
  ...
  if (
    ...
    try {
      ...
      deletionCount++;
      console.log(`🗑️ Deleted message from: ${msg.from.first_name}. Total deletions: ${deletionCount}`);
    } catch (error) {}
  }
});
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (!isAdmin(chatId)) {
    await bot.answerCallbackQuery(callbackQuery.id, { text: "⚠️ أنت لست الأدمن!" });
    return;
  }
  ...
  switch (data) {
    // قوائم التنقل
    case 'menu_main':
        bot.editMessageText(mainAdminMenu.text, { chat_id: chatId, message_id: msg.message_id, ...mainAdminMenu.options });
        break;
    case 'menu_stats':
        bot.editMessageText(statsMenu.text, { chat_id: chatId, message_id: msg.message_id, ...statsMenu.options });
        break;
    case 'menu_protection':
        bot.editMessageText(protectionMenu.text, { chat_id: chatId, message_id: msg.message_id, ...protectionMenu.options });
        break;
    case 'menu_info':
        bot.editMessageText(infoMenu.text, { chat_id: chatId, message_id: msg.message_id, ...infoMenu.options });
        break;

    // خيارات الإحصائيات
    case 'status_bot':
        bot.sendMessage(chatId, "✅ البوت يعمل بشكل طبيعي وجاهز لاستقبال الرسائل.");
        break;
    case 'status_deletions':
        bot.sendMessage(chatId, `🔢 إجمالي الرسائل المحذوفة منذ آخر إعادة تشغيل: *${deletionCount}*`, { parse_mode: "Markdown" });
        break;

    // خيارات الحماية
    case 'list_banned':
        const keywordsList = badKeywords.length > 0 ? badKeywords.join("\n") : "لا توجد كلمات ممنوعة حالياً.";
        bot.sendMessage(chatId, `📖 *الكلمات الممنوعة حالياً:*\n\n${keywordsList}`, { parse_mode: "Markdown" });
        break;
    case 'add_banned_word':
        adminState = 'awaiting_word_to_add';
        bot.sendMessage(chatId, "✍️ أرسل الكلمة التي تريد إضافتها إلى القائمة. للإلغاء، أرسل /cancel.");
        break;
    case 'remove_banned_word':
        adminState = 'awaiting_word_to_remove';
        bot.sendMessage(chatId, "🗑️ أرسل الكلمة التي تريد حذفها من القائمة. للإلغاء، أرسل /cancel.");
        break;
    case 'toggle_deletion':
        isDeletionEnabled = !isDeletionEnabled;
        const status = isDeletionEnabled ? " مفعل" : "معطل";
        bot.sendMessage(chatId, `✅ تم تحديث حالة الحذف. الحذف الآن *${status}*.`);
        break;

    // خيارات المعلومات
    case 'info_chat_id':
        bot.sendMessage(chatId, `🆔 معرف الدردشة الخاص بك هو: \`${chatId}\``, { parse_mode: "Markdown" });
        break;
    case 'info_dev':
